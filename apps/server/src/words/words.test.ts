...
describe("word packs", () => {
  ...
  it("never selects disabled words", () => {
    ...
    const food = WORD_PACKS["world-food"];
    const target = food[0]!;
    target.enabled = false;
    try {
      for (let i = 0; i < 200; i++) {
        const w = pickWord("world-food", used);
        expect(w.word).not.toBe(target.word);
        if (used.size >= enabledWords("world-food").length) used.clear();
      }
    } finally {
...
describe("pickWord", () => {
  it("is deterministic with an injected rng", () => {
    const used = new Set<string>();
    const first = pickWord("world-food", used, () => 0); // index 0
    expect(first.word).toBe(enabledWords("world-food")[0]!.word);
  ...
  it("does not repeat within a session until exhausted", () => {
    ...
    const poolSize = enabledWords("world-food").length;
  ...
  it("resets and reuses once the pool is exhausted", () => {
    const used = new Set<string>();
    const poolSize = enabledWords("world-food").length;
    for (let i = 0; i < poolSize; i++) pickWord("world-food", used);
    // pool now exhausted; next pick should reset `used` and still return a word
    const next = pickWord("world-food", used);
  ...
  it("throws when a category has zero enabled words", () => {
    const pack = WORD_PACKS["world-movies"];
    const saved = pack.map((w) => w.enabled);
    pack.forEach((w) => (w.enabled = false));
    try {
      expect(() => pickWord("world-movies", new Set())).toThrow(/no enabled words/i);
    } finally {
