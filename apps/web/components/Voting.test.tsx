...
describe("Voting", () => {
  ...
  it("excludes eliminated players as targets", () => {
    const r: RoomSummary = {
      ...room,
      players: room.players.map((p) => (p.id === "p3" ? { ...p, isEliminated: true, isSpectator: false, score: 0, colorIndex: 0 } : p)),
    };
  ...
  it("shows spectator state for an eliminated voter", () => {
    ...
    const r: RoomSummary = {
      ...room,
      players: room.players.map((p) => (p.id === "p2" ? { ...p, isEliminated: true, isSpectator: false, score: 0, colorIndex: 0 } : p)),
    };
    render(<Voting room={r} />);
